...
bool ScopeTable::insert(const string &name, const string &type, std::ofstream& logfile)
{
    // First check if symbol already exists
    ...
    while (current != nullptr)
    {
        if (current->getName() == name)
        {
            ...
            logfile << "< " << name << " : " << type << " > already exists in ScopeTable# " 
                   << id << " at position " << index << ", " << chainPos << std::endl << std::endl;
            return false;
        }
        current = current->getNext();
        chainPos++;
...
// New function to print the scope table to an ofstream
void ScopeTable::print(std::ofstream& outFile) const
{
    outFile << "ScopeTable # " << id << std::endl;
