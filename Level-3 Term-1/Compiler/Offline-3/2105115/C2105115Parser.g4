...
start : program
	{
        writeIntoparserLogFile("Symbol Table after completing parsing:");
...
program returns [std::string text]
    : p=program u=unit {
            $text = $p.text + "\n" + $u.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($u.start->getLine()) + ": program : program unit");
            writeIntoparserLogFile("");
	...
	| u=unit {
            $text = $u.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($u.start->getLine()) + ": program : unit");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
...
unit : v=var_declaration {
            writeIntoparserLogFile(std::string("Line ") + std::to_string($v.start->getLine()) + ": unit : var_declaration");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($v.text);
            writeIntoparserLogFile("");
        }
     | fd=func_declaration {
            writeIntoparserLogFile(std::string("Line ") + std::to_string($fd.start->getLine()) + ": unit : func_declaration");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($fd.text);
            writeIntoparserLogFile("");
        }
     | fdef=func_definition {
            writeIntoparserLogFile(std::string("Line ") + std::to_string($fdef.start->getLine()) + ": unit : func_definition");
            writeIntoparserLogFile("");
...
func_declaration returns [std::string text]
        : t=type_specifier i=ID lp=LPAREN p=parameter_list rp=RPAREN s=SEMICOLON {
...
func_definition returns [std::string text]
         : t=type_specifier i=ID lp=LPAREN {
            ...
            bool inserted = symbolTable.insert($i->getText(), currentTypeSpecifier, parserLogFile);
            if (!inserted) {
                writeIntoErrorFile("Error in func_definition");
                syntaxErrorCount++;
            }
...
declaration_list returns [std::string text]
 		  : dl=declaration_list cm=COMMA id=ID {
 		      ...
 		      if (!inserted) {
 		          writeIntoErrorFile(std::string("Line# ") + std::to_string($id->getLine()) + 
 		                              ": Redefinition of variable '" + $id->getText() + "'");
 		          syntaxErrorCount++;
 		      }
 		  ...
 		  | dl=declaration_list cm=COMMA id=ID LTHIRD ci=CONST_INT RTHIRD {
 		      ...
 		      if (!inserted) {
 		          writeIntoErrorFile(std::string("Line# ") + std::to_string($id->getLine()) + 
 		                              ": Redefinition of array '" + $id->getText() + "'");
 		          syntaxErrorCount++;
 		      }
...
statements returns [std::string text]
	   ...
	   | ss=statements s=statement {
            $text = $ss.text + "\n" + $s.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($s.start->getLine()) + ": statements : statements statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
...
statement returns [std::string text]
      : v=var_declaration {
            $text = $v.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($v.start->getLine()) + ": statement : var_declaration");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | es=expression_statement {
            $text = $es.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($es.start->getLine()) + ": statement : expression_statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | c=compound_statement {
            $text = $c.text;
            writeIntoparserLogFile(std::string("Line ") + std::to_string($c.start->getLine()) + ": statement : compound_statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | f=FOR lp=LPAREN e1=expression_statement e2=expression_statement exp=expression rp=RPAREN s=statement {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($f->getLine()) + ": statement : FOR LPAREN expression_statement expression_statement expression RPAREN statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | i=IF lp=LPAREN exp=expression rp=RPAREN s=statement {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($i->getLine()) + ": statement : IF LPAREN expression RPAREN statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | i=IF lp=LPAREN exp=expression rp=RPAREN s1=statement el=ELSE s2=statement {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($i->getLine()) + ": statement : IF LPAREN expression RPAREN statement ELSE statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | w=WHILE lp=LPAREN exp=expression rp=RPAREN s=statement {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($w->getLine()) + ": statement : WHILE LPAREN expression RPAREN statement");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | p=PRINTLN lp=LPAREN id=ID rp=RPAREN sm=SEMICOLON {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($p->getLine()) + ": statement : PRINTLN LPAREN ID RPAREN SEMICOLON");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
	  | r=RETURN exp=expression sm=SEMICOLON {
            ...
            writeIntoparserLogFile(std::string("Line ") + std::to_string($r->getLine()) + ": statement : RETURN expression SEMICOLON");
            writeIntoparserLogFile("");
            writeIntoparserLogFile($text);
            writeIntoparserLogFile("");
        }
